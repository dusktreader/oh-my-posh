...
Filler Template fills the same role as Filler, but allows you to specify a template to tweak the text used
as filler. This template behaves the same as Segment templates, however, fewer properties are available.
Properties available for the Filler Template are:

| Name        | Type   | Description
| ----------- | ------ | ------------------------------------------------------------------
| `.Overflow` | `text` | if no overflow was needed, this is "". Otherwise "hide" or "break"
...
<Config
  data={{
    block: {
      alignment: "right",
      overflow: "hide",
      filler_template: "{{ if .Overflow }} {{ else }}-{{ end }}",
    },
  }}
/>
